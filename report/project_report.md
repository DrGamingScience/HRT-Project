...
*   **Mô hình đề xuất:** CNN Encoder (ResNet18) + BiLSTM Context Layer + Cơ chế Bahdanau Attention + RNN Decoder (GRU).

---

## 2. KIẾN TRÚC MÔ HÌNH (MODEL ARCHITECTURE)

### 2.1. Mô hình Đề xuất (Attention Model)
Kiến trúc mô hình được xây dựng theo hướng Encoder–Decoder kết hợp cơ chế chú ý Bahdanau để căn chỉnh động đặc trưng ảnh và chuỗi ký tự đầu ra:
...
### 2.2. Sự cần thiết của bộ giải mã độc lập (inference/)
...
2.  **Độc lập về giải thuật giải mã (Decoding Algorithms):** Việc chuyển đổi từ ma trận xác suất sang chuỗi chữ viết có nghĩa (ví dụ: tìm kiếm tham lam Greedy hoặc tìm kiếm chùm Beam Search) là các thuật toán logic độc lập với mô hình học sâu.
3.  **Tái sử dụng mã nguồn và dễ bảo trì (Modular Design):** Cả tiến trình đánh giá tập test (`evaluate_attention.py`) và tiến trình dự đoán ảnh đơn lẻ (`predict_attention.py`) đều cần dịch ma trận xác suất thành chữ. Việc tách các giải thuật này vào thư mục `inference/` giúp tránh trùng lặp code, đồng thời cho phép dễ dàng nâng cấp hoặc thay thế thuật toán giải mã (ví dụ: chuyển từ Greedy sang Beam Search) mà không cần sửa đổi cấu trúc của mô hình hay các file chạy chính.
...
Kết quả thực tế thu được từ checkpoint của mô hình Attention tốt nhất trên 9,684 mẫu kiểm thử:

| Chỉ số (Metrics) | Mô hình Attention (Phase 1) |
|---|---|
| **Word Accuracy (WA)** | **83.09%** |
| **Character Error Rate (CER)** | **7.06%** |
| **Normalized Edit Distance (NED)** | **94.21%** |
| **Epoch đạt kết quả tốt nhất** | Epoch 62 / 80 |
| **Dung lượng file checkpoint** | ~91.0 MB |
...
2.  **Tầm quan trọng của BiLSTM Context Layer:**
